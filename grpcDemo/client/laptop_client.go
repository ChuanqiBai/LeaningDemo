...
func (laptopClient *LaptopClient) SearchLaptop(filter *pb.Filter) {
	...
	for {
		...
		log.Print("-found: ", laptop.Id)
		log.Print(" +brand: ", laptop.Brand)
		log.Print(" +name: ", laptop.Name)
		log.Print(" +cpu cores: ", laptop.GetCpu().GetNumberCores())
		log.Print(" +cpu min ghz: ", laptop.GetCpu().GetMinGhz())
		log.Print(" +ram: ", laptop.Ram)
		log.Print(" +price: ", laptop.PriceUsd)
	}
}
...
func (laptopClient *LaptopClient) UploadImage(laptopID string, imagePath string) {
	...
	for {
		...
		if err != nil {
			log.Fatal("cannot read chunk data to buffer: ", err)
		}
	...
	if err != nil {
		log.Fatal("cannot receive server response: ", err)
	}
	log.Printf("upload image success with id:%s, size:%d", res.GetId(), res.GetSize())
}
...
func (laptopClient *LaptopClient) RateLaptop(laptopIDs []string, scores []float64) error {
	...
	for i, laptopID := range laptopIDs {
		...
		err := stream.Send(req)
		if err != nil {
			return fmt.Errorf("cannot send rating request: %v - %v", err, stream.RecvMsg(nil))
		}
