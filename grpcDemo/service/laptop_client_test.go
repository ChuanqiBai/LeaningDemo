...
func TestClientCreateLaptop(t *testing.T) {
	...
	//check the saved laptop is the same as the on we send
...
func TestClientUploadImage(t *testing.T) {
	...
	// require.NoError(t, os.Remove(savedImagePath))	//会报错，其他进程占用该文件
}
...
func startTestLaptopServer(t *testing.T, laptopStore service.LaptopStore, imageStore service.ImageStore, ratingStore service.RatingStore) string {
	...
	listener, err := net.Listen("tcp", ":0") //":0" 代表分配任意可用的port
	require.NoError(t, err)

	go grpcServer.Serve(listener) //因为该语句是阻塞式的，所以应该放到协程去跑
