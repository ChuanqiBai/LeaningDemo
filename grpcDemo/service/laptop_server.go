...
func (server *LaptopServer) SearchLaptop(req *pb.SearchLaptopRequest,
	...
	err := server.laptopStore.Search(stream.Context(), filter, func(laptop *pb.Laptop) error {
		...
		log.Printf("send laptop to the client whit id:%s", laptop.Id)
		return nil
	})
...
func (server *LaptopServer) UploadImage(stream pb.LaptopService_UploadImageServer) error {
	...
	for {
		...
		log.Printf("received a chunk data with size: %d", size)
		if imageSize > maxIamgeSize {
			return logError(status.Errorf(codes.InvalidArgument, "image is too large: %d>%d", imageSize, maxIamgeSize))
		}
